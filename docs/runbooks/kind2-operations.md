...
5. `SetForceCloseTimestamp` (tag 87) — admin + council co-signed. Unix-seconds, **set conservatively BEFORE Polymarket's expected resolution** — far enough ahead that even if resolution arrives early, our force-close still fires first and settles at the captured TWAP, so a live position never sees Polymarket's terminal 0/1 jump (the core force-close-before-resolution property). The wrapper enforces a hard floor of `MIN_FUTURE_SECS` (48h from configuration) and a ceiling of `MAX_FUTURE_SECS` (1 year); within that window the council picks the conservative pre-resolution timestamp. The K4' cranker fires at this clock regardless of Polymarket resolution state. There is no undo.
...
- **Force-close fires regardless of Polymarket resolution state.** The K4' cranker uses only the on-chain `force_close_unix_timestamp` gate. The council's responsibility is to set that timestamp **before** Polymarket's expected resolution — with enough margin that even an early resolution still lands after our force-close — so we always settle at the captured TWAP and a live position never reaches Polymarket's terminal 0/1 jump. The race to avoid is "Polymarket resolves before we force-close"; setting the timestamp conservatively pre-resolution closes it. (Do not set it *after* expected resolution — that inverts the core property and exposes positions to the binary jump.) The deferred V2 design hooks resolution-state into the gate directly.
